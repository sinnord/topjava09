...
это ссылка на метод, сокращенная форма лямбды. IDEA иногда выделяет желтым и предлагает замену, например
`m->m.getCalories()` заменяет на `Meal::getCalories` (соглашаться по `Alt+Enter`).
...
#### Реализовать сервлет с отображением в таблице списка еды (в памяти и без учета пользователя, поле `user` в класс `Meal` НЕ добавляем)
