...
В Optional нужно делать реализацию хранения  многопоточной (оно одно на всех и шарится между пользователями). Cессии пока не используем  (начнутся, когда будет прикручивать авторизацию).
...
- Перед чекином проверяйте свой ченджлист (Ctrl+D на файле из Local Change - посмотреть что поменялось). Если тым только пробелы/переводы строк, не нодо его комитить, делайте файлу revert.
- ПишитеПривыкайте писать коментарии к чекину: одной фразой что вы сделали в нем. Нарпимер: Meals CRUD implementation.
